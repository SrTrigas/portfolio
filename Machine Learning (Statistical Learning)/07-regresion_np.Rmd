...
Otros paquetes de R incluyen más funcionalidades (`sm`, `locfit`, `r citepkg("npsp","https://rubenfcasal.github.io/npsp")`...), pero hoy en día el paquete `r citepkg("np","https://github.com/JeffreyRacine/R-Package-np")` es el que se podría considerar más completo.
...
knots <- seq(min(x), max(x), len = nknots + 2)
fint <- cut(x, knots, include.lowest = TRUE)
fit <- lm(y ~ x*fint)
plot(x, y, col = 'darkgray')

pdata <- split(data.frame(x = x, pred = predict(fit)), fint)
lapply(pdata, function(d) lines(d$x, d$pred))
knots <- knots[-c(1, nknots + 2)]
abline(v = knots, lty = 3)
```

De esta forma sin embargo habrá discontinuidades en los puntos de corte, pero podrían añadirse restricciones adicionales de continuidad (o incluso de diferenciabilidad) para evitarlo (e.g. paquete `r citepkg("segmented")`).
...
Puede ser más cómodo emplear el paquete `r citepkg("modelr", "https://modelr.tidyverse.org")` (emplea gráficos `ggplot2`) para trabajar con modelos y predicciones.
...
```{r fig.height=6, fig.width=12, out.width = "90%"}
...
Para ajustar un modelo *single index* también se podría emplear la función `npindex()` del paquete  `r citepkg("np","https://github.com/JeffreyRacine/R-Package-np")` [que implementa el método de @ichimura1993, considerando un estimador local constante], aunque en este caso ni el tiempo de computación ni el resultado es satisfactorio:
